                ...
                <tr>
                    ...
                    <td class="item-quantity">
                        <div class="box-quantity">
                            <button type="button" class="minus decrease-btn{{$item[0]['id']}}" value="-"></button>
                            @foreach ($carts as $i)
                                @if ($i['id_prod'] == $item[0]['id'])
                                    <input id="QtyInput-sinh{{$item[0]['id']}}" type="text" class="qty" step="1" min="0" max
                                        value="{{ $i['qty'] }}" inputmode="numeric">
                                @endif
                            @endforeach
                            <button type="button" class="plus increase-btn{{$item[0]['id']}}" value="+"></button>
                        </div>
                    </td>
